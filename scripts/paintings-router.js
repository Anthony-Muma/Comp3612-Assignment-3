...
function handlePaintingId(paintings, app) {
    app.get('/api/painting/:id', (req, resp) => {
        ...
        if (matches.length > 0) {
            ...
            resp.json(jsonMessage(`Couldn't find a match with id '${id}'`));
        }
    });
}
...
function handlePaintingMinMax(paintings, app) {
    app.get('/api/painting/year/:min/:max', (req, resp) => {
        ...
        if (matches.length > 0) {
            ...
            resp.json(jsonMessage(`Couldn't find a match with years between '${min}' to '${max}'`));
        }
    });
}
...
function handlePaintingTitleName(paintings, app) {
    app.get('/api/painting/title/:text', (req, resp) => {
        ...
        if (matches.length > 0) {
            ...
            resp.json(jsonMessage(`Couldn't find a match with titles including '${text}'`));
        }
    });
}
...
function handlePaintingColorName(paintings, app) {
    app.get('/api/painting/color/:name', (req, resp) => {
        ...
        if (matches.length > 0) {
            ...
            resp.json(jsonMessage(`Couldn't find a match with color '${name}'`));
        }
    });
}
